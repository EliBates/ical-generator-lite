...
export default class ICalEvent {
    ...
    repeating(
        repeating?: ICalRepeatingOptions | ICalRRuleStub | string | null
    ): this | ICalEventInternalRepeatingData | ICalRRuleStub | string | null {
    ...
    location(
        ...
        if (
            ...
            (location?.geo &&
                (!isFinite(location.geo.lat) || !isFinite(location.geo.lon)))
        ) {
            throw new Error(
    ...
    toString(): string {
        ...
        if (this.data.allDay) {
            g +=
                "DTSTART;VALUE=DATE:" +
                formatDate(this.calendar.timezone(), this.data.start, true) +
                "\r\n";
            if (this.data.end) {
                g +=
                    "DTEND;VALUE=DATE:" +
                    formatDate(this.calendar.timezone(), this.data.end, true) +
                    "\r\n";
            }
